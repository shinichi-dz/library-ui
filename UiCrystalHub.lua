function GetUi() 
    ...
    if game.CoreGui:FindFirstChild("Crystal Hub GUI") then
        for a, v in ipairs(game.CoreGui:GetChildren()) do
            if v.Name == "Crystal Hub GUI" then
                v:Destroy()
            end
        end
    end
...
local b = {
    ...
    ["Background Main Color"] = Color3.fromRGB(46, 42, 58),
    ["Background 1 Color"] = Color3.fromRGB(50, 46, 64),
    ["Background 1 Transparency"] = 0,
    ["Background 2 Color"] = Color3.fromRGB(60, 56, 74),
    ["Background 3 Color"] = Color3.fromRGB(54, 50, 68),
    ["Background Image"] = "",
    ["Page Selected Color"] = Color3.fromRGB(131, 181, 255),
    ["Section Text Color"] = Color3.fromRGB(131, 181, 255),
    ["Section Underline Color"] = Color3.fromRGB(131, 181, 255),
...
function SaveCustomUISettings()
    local j = game:GetService("HttpService")
    if not isfolder("Crystal Hub") then
        makefolder("Crystal Hub")
    end
    writefile("Crystal Hub/" .. k, j:JSONEncode(CorrectTable(e)))
end
function ReadCustomUISetting()
    local l, m =
        ...
        function()
            ...
            local n = j:JSONDecode(readfile("Crystal Hub/" .. k))
...
u.NotiGui.Name = "Crystal Hub Notification"
...
function t.CreateNoti(H)
    ...
    local g = V .. "," .. W .. "," .. X
    Q.Text = '<font color="rgb(' .. g .. ')">Crystal Hub</font> ' .. getgenv().TitleNameNoti
    table.insert(
        c["Title Text Color"],
        function()
            local V = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[1])
...
function t.CreateMain(H)
    local Z = tostring(H.Title) or "Crystal Hub"
    ...
    ImageButton.Image = "rbxassetid://129781592728096"
    ...
    function u.ReloadMain(v)
        ...
        local X = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[3])
        local g = V .. "," .. W .. "," .. X
        ad.Text = '<font color="rgb(' .. g .. ')">Crystal Hub</font> ' .. getgenv().MainDesc
        table.insert(
            c["Title Text Color"],
            function()
                aa.ImageColor3 = getgenv().UIColor["Title Text Color"]
...
local Main = Library.CreateMain({Title = 'Blox Fruit', Desc = ''})

local Page1 = Main.CreatePage({Page_Name = 'Gerenal', Page_Title = 'Gerenal Tab'})

Library.CreateNoti({
    Title = nil,
    Desc = "Crystal Hub" .. "Isekai Lon Hub",
    ShowTime = 5,
})
...
Section1.CreateDropdown({Title = 'Dropdown', List = Options, Search = false, Selected = false}, function(value)
    print(Value)
end)

Section1.CreateButton({Title = 'Button'}, function()
    print("Button Clicked!")
end)

Section1.CreateButton({Title = 'Button', Desc = 'Desc For Button'}, function()
    print("Button Clicked!")
end)


Section1.CreateToggle({Title = "Toggle"}, function(value)
    print(Value)
end)

local Section2 = Page1.CreateSection('Main')

local Label1 = Section2.CreateLabel({Title = "Dit Me May: "})
Label1.SetText("Dit Me May: :".." ".."Me May Beo")

Section2.CreateToggle({Title = "Toggle", Desc = 'Desc For Toggle', Default = false}, function(value)
    print(Value)
end)

Section2.CreateSlider({Title = "Slider", Min = 0, Max = 100, Default = 25 }, function(value)
    print(Value)
end)

Section2.CreateBox({Title = 'Job Id Placed', Placeholder = "Input iob id notify in server script ", Number_Only = false }, function(value)
    print(Value)
end)
